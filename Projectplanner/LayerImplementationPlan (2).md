...
### Objective
Formalize infrastructure elements (walls, columns, crane runways) and the generalized Opening model (doors, hatches, aisles as instances with different capacity). Create spatial elements (zones, aisles, restricted areas) with dedicated renderers.
...
- State indication: green=Open, red=Closed, gray=Locked

**Task 2.7a: Create Opening Tool**
- Tool for placing openings in walls or between zones
- Click on wall to insert opening (auto-creates gap)
- Click between two zones to create freestanding opening
- Property panel sets: opening type, capacity, dimensions
- Subtype selection: Door, Hatch, Manhole, Aisle, Gate
- Subtype properties appear based on selection
- Default capacity: 1 for Door/Hatch/Manhole, 0 for Aisle
- Auto-links FromZoneId and ToZoneId from adjacent zones
...
| T2.9 | Door subtype has SwingDirection | Property exists and serializes |
| T2.10 | Hatch subtype has LadderTime | Property exists and serializes |
| T2.11 | Opening tool creates correct subtype | Select Door, get Door properties |
| T2.12 | Opening auto-links zones | FromZoneId and ToZoneId set on placement |
| T2.13 | PrimaryAisleData has Spatial layer | Layer == Spatial |
| T2.14 | RestrictedAreaData has Spatial layer | Layer == Spatial |
| T2.15 | Hide Infrastructure hides walls and openings | Toggle off, elements disappear |
| T2.16 | Hide Spatial hides zones | Toggle off, zones disappear |
| T2.17 | Hidden layer elements not selectable | Click passes through |
| T2.18 | Locked layer elements not editable | Cannot move or modify |
| T2.19 | Save includes openings with subtypes | Save, reload, door properties intact |
| T2.20 | Opening connects two zones | FromZoneId and ToZoneId valid |

### Completion Criteria
- [ ] Opening base model with capacity and state
- [ ] Capacity = 0 creates unconstrained opening
- [ ] Capacity >= 1 creates constrained opening
- [ ] All openings have state machine (Open/Closed/Locked/Emergency)
- [ ] Door, Hatch, Manhole, Gate subtypes implemented
- [ ] Each subtype has specific properties
- [ ] Temporary opening model working
- [ ] Opening tool places openings
- [ ] Opening renders with type-specific symbol
- [ ] Opening state visually indicated (color)
- [ ] Opening auto-links to adjacent zones
- [ ] Primary aisle model and renderer working
- [ ] Restricted area model and renderer working
- [ ] Infrastructure renderer handles all elements
...
3. Add openings to walls (doors with capacity=1, wide aisle with capacity=0)
4. Create zones on Spatial layer (L1)
5. Verify openings connect adjacent zones (FromZoneId, ToZoneId populated)
6. Add primary aisles
7. Place equipment on Equipment layer (L2)
8. Show clearance zones
9. Group equipment into cells on LocalFlow layer (L3)
10. Draw conveyors between equipment
11. Create AGV network on GuidedTransport layer (L4)
12. Add AGV stations linked to equipment
13. Define crane coverage on OverheadTransport layer (L5)
14. Draw forklift aisles on FlexibleTransport layer (L6)
15. Generate pedestrian mesh on Pedestrian layer (L7)
16. Save layout
17. Close and reopen
18. Verify all elements present including opening subtypes
...
5. Verify openings table has capacity and state columns
6. Verify door_properties table links to openings
7. Verify foreign keys correct
8. Verify element counts match

**Scenario D: Opening State Behavior**
1. Create layout with two zones connected by door (capacity=1)
2. Create layout with two zones connected by wide aisle (capacity=0)
3. Verify both openings have state = Open by default
4. Change door state to Closed
5. Verify simulation would block traversal (state check)
6. Change aisle state to Closed
7. Verify simulation would block traversal (even though capacity=0)
8. Change door state to Locked
9. Verify state transition works
10. Save and reload, verify states preserved
...
- [ ] Models/Layers/InfrastructureModels.cs (walls, columns, runways)
- [ ] Models/Layers/OpeningModels.cs (Opening base, Door, Hatch, Manhole, Gate, TemporaryOpening)
- [ ] Models/Layers/SpatialModels.cs (zones, aisles, restricted areas)
- [ ] Models/Layers/LocalFlowModels.cs (conveyor, direct path)
- [ ] Models/Layers/GuidedTransportModels.cs (AGV paths, waypoints, stations)
- [ ] Models/Layers/OverheadTransportModels.cs (cranes, coverage, handoffs)
- [ ] Models/Layers/FlexibleTransportModels.cs (forklift aisles, staging, crossings)
- [ ] Models/Layers/PedestrianModels.cs (walkable mesh, exclusions)
...
### Tests (75+ tests)
- [ ] Week 1: 9 tests
- [ ] Week 2: 20 tests (includes opening model)
- [ ] Week 3: 10 tests
- [ ] Week 4: 10 tests
- [ ] Week 5: 10 tests
- [ ] Week 6: 10 tests
...
| 2 | L0 + L1 | Infrastructure + Spatial + Opening hierarchy | 20 |
...
**Total: 6 weeks, 75+ tests, 30+ files**

---

## Opening Model Reference

Quick reference for the generalized Opening model:
...
├── All openings have state machine
│
...
│   ├── Door (swing, access control)
│   ├── Hatch (vertical, ladder time)
│   ├── Manhole (confined space)
│   └── Gate (vehicle barrier)
│
└── TemporaryOpening
    └── Exists only during assembly state range
```

### Key Insight
Capacity = 0 does not mean "no model." It means "no token required." The opening still has state and can be closed. A wide aisle can be blocked for crane operations just like a door.

### Simulation Behavior
1. Check state (all openings) — if not Open, block
2. Check entity type and dimensions
3. If capacity > 0: acquire token
4. Delay by traversal time
5. If capacity > 0: release token
